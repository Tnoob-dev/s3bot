...
from . import ErrorCode, __app_name__
from .errors import AbortError
from .s3 import get_real_url, reserve_url
from .util import decode_content, generate_token, shorten_name, tqdm_logging
...
class ToDusClient:
    ...
    def auth_ua(self) -> str:
        """User Agent used for authentication (App móvil)."""
        return f"ToDus {self.version_name} Auth"

    @property
    def web_ua(self) -> str:
        """User Agent usado por la web de ToDus (Firefox)."""
        return "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:149.0) Gecko/20100101 Firefox/149.0"
    ...
    def headers_auth(self) -> Dict[str, str]:
        """Headers para autenticación estilo app móvil (protobuf)."""
        return {
            "Host": "auth.todus.cu",
    ...
    @property
    def headers_auth_web(self) -> Dict[str, str]:
        """Headers para autenticación estilo web (octet-stream)"""
        return {
            "Host": "auth.todus.cu",
            ...
            "X-Requested-With": "XMLHttpRequest",  # ← ¡ESTO FALTABA! CLAVE
            "Origin": "https://web.todus.cu",
            "Referer": "https://web.todus.cu/",
    ...
    def task_validate_code(self, phone_number: str, code: str) -> str:
        ...
        # 🔥 EXTRAER Y GUARDAR EL TOKEN DE SESIÓN (ajusta el índice según la respuesta real)
        # La respuesta probablemente contiene el token en los primeros bytes
        if len(resp.content) > 20:
            # Esto es un ejemplo: ajusta según la estructura real de la respuesta
            self.session_token = resp.content[10:52].decode('latin-1', errors='ignore')

        print(f"\n🔍 Respuesta users.register:")
        print(f"Status: {resp.status_code}")
        print(f"Content (hex): {resp.content.hex()}")
        print(f"Content (repr): {repr(resp.content)}")
        print(f"Content (decoded): {decode_content(resp.content)[:200]}")
    ...
    def _task_login_legacy(self, phone_number: str, password: str) -> str:
        """Login legacy con password (para compatibilidad)."""
        token = ""
        headers = self.headers_auth  # Usa headers de app móvil para legacy
        data = (
            b"\n\n"
            + phone_number.encode("utf-8")
            + b"\x12\x96\x01"
    ...
    def task_login(self, phone_number: str, password: str) -> str:
        """Login usando el token de sesión obtenido en validate_code."""
        token = ""
        
        # Si no tenemos session_token, intentamos con el flujo antiguo (fallback)
        if not self.session_token:
            logger.warning("No hay session_token, usando flujo con password")
            return self._task_login_legacy(phone_number, password)
        
        headers = self.headers_auth_web
        # Payload SIMPLE como la web: phone + session_token + version
        data = (
            b"\n\n"
            + phone_number.encode("utf-8")
            + b"\x12"
            + len(self.session_token).to_bytes(1, 'big')  # Length del token
            + self.session_token.encode("latin-1")         # Token real
            + b"\x1a\x05"
            + self.version_code.encode("utf-8")
        )
