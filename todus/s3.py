...
def _get_socket() -> ssl.SSLSocket:
    """
    Crea una conexión SSL segura al servidor IM de ToDus.
    Compatible con Python 3.12+ (usa SSLContext en vez de wrap_socket).
    """
    # Crear contexto SSL moderno (reemplazo de ssl.wrap_socket)
    context = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
    context.check_hostname = False
    context.verify_mode = ssl.CERT_NONE  # ToDus usa certificados propios/self-signed
    ...
    # Envolver con SSL
    ssl_socket = context.wrap_socket(sock, server_hostname=HOST_IM)
    ssl_socket.connect((HOST_IM, 1756))

    # Enviar cabecera del stream XMPP
    ssl_socket.send(
        b"<stream:stream xmlns='jc' to='im.todus.cu' xmlns:stream='x1' version='1.0'>"
    )
...
def _negociate_start(
    ...
    if response == "<ok xmlns='x2'/>":
        data = b"<stream:stream xmlns='jc' to='im.todus.cu' xmlns:stream='x1' version='1.0'>"
        ssl_socket.send(data)
        return True
